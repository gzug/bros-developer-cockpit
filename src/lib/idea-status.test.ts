...
test("idea status labels stay human-readable", () => {
  expect(getIdeaStatusLabel("requested")).toBe("Shipping requested");
  expect(getIdeaStatusLabel("live")).toBe("Checked on the phone");
});
...
test("idea next steps explain the current operator responsibility", () => {
  ...
  expect(getIdeaNextStep("blocked")).toContain("inspect");
});
...
test("idea timeline marks current progress without inventing extra progress", () => {
  ...
  expect(live.every((step, index) => (index < live.length - 1 ? step.state === "complete" : step.state === "current"))).toBe(true);
});
